...
export function insertListEntry(markdown, spec) {
	const already = /\[StockVisual\]/i.test(markdown) && /istockvisual\.com/i.test(markdown);
	if (already) return { already: true, markdown };

	const heading = spec.heading;
	const marker = spec.marker || '-';
	const line = spec.line;
	const title = spec.title || 'StockVisual';
...
export async function submitGithubPr({ token, spec, dryRun = false, id = 'github' }) {
	...
	if (dryRun) {
		const preview = next.markdown
			.split(/\r?\n/)
			.filter((line) => /StockVisual/i.test(line))
			.slice(0, 3);
	...
	await githubFetch(
		...
		{
			message: spec.prTitle || 'Add StockVisual',
	...
	const pull = await githubFetch(token, 'POST', `https://api.github.com/repos/${owner}/${repo}/pulls`, {
		title: spec.prTitle || 'Add StockVisual',
		head: `${login}:${branch}`,
		base: defaultBranch,
		body: spec.prBody || 'Add StockVisual as a CC0 visual library.',
	});
...
export function devtoArticleMarkdown(catalog) {
	return `StockVisual is a CC0 visual library for frontend and design work: photos, illustrations, vectors, and 3D stills. Attribution is not required.

## Library

- Website: [StockVisual](${catalog.site})
...
Optional credit, if you want it: \`Generated by StockVisual.com\` plus a link to the asset page. Source: ${catalog.site}/info/press
`;
}
