...
import { LICENSE_PATH, ORG_NAME } from '../lib/seo';
...
function attributionSnippet(format: AttrFormatId, title: string, url: string) {
	...
	switch (format) {
		...
		case 'html-paren':
			return `(<a href="${url}">${safeTitle}</a>)`;
		case 'html-source':
			return `Source: <a href="${url}">${ORG_NAME}</a>`;
		case 'markdown':
			return `[${title}](${url})`;
		case 'plain':
			return buildPhotoAttribution(url);
	}
}
	...
	return (
		<div className="download-auth-modal" role="presentation">
			...
			>
				...
				<p className="share-love__thanks">Thanks for crediting {ORG_NAME}.</p>
			</div>
		</div>
	);
}
